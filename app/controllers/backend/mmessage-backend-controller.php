...
class MMessage_Backend_Controller extends IG_Request {
	...
	function admin_menu() {
		add_menu_page( __( 'Messaging', mmg()->domain ), __( 'Messaging', mmg()->domain ), 'manage_options', mmg()->prefix . 'main', array(
			&$this,
			'main'
		), 'dashicons-email-alt' );
		add_submenu_page( null, __( 'View Messages', mmg()->domain ), __( 'View Messages', mmg()->domain ), 'manage_options', mmg()->prefix . 'view', array(
			&$this,
			'view'
		) );
		add_submenu_page( mmg()->prefix . 'main', __( 'Settings', mmg()->domain ), __( 'Settings', mmg()->domain ), 'manage_options', mmg()->prefix . 'setting', array(
			&$this,
			'setting'
		) );
	}
